...
def get_string_from_dict(d:str,
	...
	ret = key_key_separator.join([f'{key}{key_value_separator}{d[key]}'for key in d.keys()])
	return ret
...
def xstr(x,
	...
	if isinstance(x, str):
		return x
	if isinstance(x, int):
		return p+format_int(x) # int
	if isinstance(x, float):
		return p+format_float(x, n_decimals, remove_zero) # float
